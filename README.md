...
With all these visual features, the frame rate on this path tracer sure has become abysmal. I believe at my low point, I was taking ~3 samples per pixel, per second, for a relatively low-poly scene. For the above spaceship scene, a reasonable image would take a couple minutes to render, and I had already done a few optimizations by then. Let's discuss some of the ways I improved the performance of the path tracer to be capable of rendering more complex scenes in reasonable times.
